...
### 結論 B-3 (実機報告) — 段階の切り分けと自動フォールバック

B-2 の修正後も実機では次が報告された。

* **強力側 (アセット差し替え) ON** → リザルト画面やバトル中のリロードを挟むと**全く動作しなくなる**
* **軽量化のみ ON** → バトル初回は開けるが、リロードすると**バトルが描写されない**

手元の再現環境 (スプライトシートの読込完了とフレーム数計算を経てからバトル UI を組み立て、
`#canvas` へ rAF 描画ループを回すスタブ) では、フレーム内リロードを 2 回挟んでも
`replaceFrame` を挟んでも両モードとも `phase: 'ready'` / 描画ループ継続 / ボタンあり で、
**再現しなかった**。実機の起動シーケンスを再現しきれていない。

推測での修正を重ねるのをやめ、**壊れうる処理を既定から外し、壊れたら自動で戻る**構造にした。

#### 段階の再定義

| スイッチ | 内容 | ゲームへの介入 |
|---|---|---|
| **バトル軽量化・高速化（安全）** | 背景画像除去・カットイン等の演出を最短化する CSS + `Game.setting.sound_flag` / `createjs.Sound.setMute` によるミュート | **JavaScript には一切介入しない。バトルは今まで通り表示され操作できる** |
| **強力モード（実験的）** | バトル canvas 非表示 + WebGL / Canvas2D 描画コール停止 + 画像の透明 GIF 置換 + 音声モジュール置換 | ゲームの描画・読込処理に介入する |

**canvas 非表示と描画コール停止を強力モード側へ移した**のが要点。バトルが見えない状態は
「壊れている」と区別が付かないので、既定で有効にしてよい挙動ではない。
安全側は prototype を 1 つも触らない。

#### 停止検知による自動フォールバック

親側に 1 秒間隔のウォッチドッグを追加した。フレームの URL が raid ルートで、
`pageBaseReady` を満たしているのに `.btn-auto` / `.btn-attack-start` が
**12 秒現れなければ**、強力モードを自動で OFF にしてログとトーストで通知する。

安全側は CSS とゲーム標準のミュートだけなので停止の原因になり得ない。
よって**安全側は自動 OFF にせず**、一度だけログを出して設定はそのままにする
(ネットワーク等の別要因で設定を勝手に失わないため)。

実測 (バトル UI を絶対に生成しない raid ルートのスタブ):

```
開始時 : { enabled: true, strong: true }
+6s    : { enabled: true, strong: true }            ← しきい値未満
+15s   : { enabled: true, strong: false,
           logs: ['バトル画面が操作可能にならなかったため強力モードを自動でOFFにしました'] }
```
